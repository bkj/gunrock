    ...
    util::Parameters &parameters)
{
    ...
    GUARD_CU(parameters.Use<uint64_t>(
        "srcs",
        util::REQUIRED_ARGUMENT | util::MULTI_VALUE | util::INTERNAL_PARAMETER,
        0,
        "Array of source vertices",
        __FILE__, __LINE__));
    ...
    OpT               op)
{
    ...
    {
        parameters.Get<std::vector<std::string> >(
            switching_paras[i], level_strings[i]);
...
template <typename OpT, typename VertexT, typename SizeT, typename ValueT, SwitchFlag FLAG>
cudaError_t Switch_Direction(util::Parameters &parameters, OpT op)
{
    cudaError_t retval = cudaSuccess;
    VertexT v = 0; SizeT s = 0; ValueT val = 0;
...
template <typename OpT, typename VertexT, typename SizeT, SwitchFlag FLAG, SwitchFlag VFLAG>
struct Switch_ValueT
{
    static cudaError_t Act(util::Parameters &parameters, OpT op)
    {
        return util::GRError(
            std::string("Unfined switch cases: ValueTFlag ") +
            std::to_string(VFLAG), __FILE__, __LINE__);
    }
};
...
struct Switch_SizeT
{
    static cudaError_t Act(
        util::Parameters &parameters, OpT op)
    {
        return util::GRError(
            std::string("Unfined switch cases: SizeTFlag ") +
            std::to_string(SFLAG), __FILE__, __LINE__);
    }
};
...
struct Switch_VertexT
{
    ...
    {
        return util::GRError(
            std::string("Unfined switch cases: VertexTFlag ") +
            std::to_string(VFLAG), __FILE__, __LINE__);
    }
};
...
{
    cudaError_t retval = cudaSuccess;
    retval = Switch_VertexT<OpT, FLAG, FLAG & VERTEXT_BASE>::
        Act(parameters, op);
    return retval;
}
